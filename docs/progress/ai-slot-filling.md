...
**Fixes from review on #507.** Both review halves (the agent review and Greptile) raised the same five points, fixed in the same PR:

- A provider's default model now goes through the same id checks as a chosen one (`slotModels`), so a default that holds the app's separator, or a control character, is a plain 422 instead of a list split in two. Test: `TestResolveAIBindings_DefaultChecked`.
- The setup page's `requires` and required-field gate counts only the fields a binding really fills (above). Before, a native slot with only a base URL field passed the UI with an account that had no base URL.
- The inline add form opens on its own only when the accounts list is known to be empty. While the list loads the row says "Loading your accounts…". On a failed read it says "Could not load your accounts." with Try again and Add an account. A form the page opened on its own closes when the list arrives with accounts.
- In a model list, an id typed but not yet added is kept on Save, after the same checks as Add.
- The model pickers refuse an id with line breaks or control characters, with a short message (`modelIdProblem`), the same rule as the brain.
