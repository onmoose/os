...
Two of those are about the catalog and are easy to mix up. `MOOSE_CATALOG_CACHE_DIR` is where proxied icons and screenshots land — never the catalog itself, which the brain holds in memory and re-fetches (`../specs/APP_STORE.md` # Failure modes). `MOOSE_CATALOG_FILE` is a local snapshot the brain reads once at startup and never writes back, for working without a reachable control plane; `make dev-app` sets it for you. A real box sets neither by hand.
