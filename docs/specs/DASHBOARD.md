...
**AI keys get an AI providers row instead of text boxes.** Fields whose manifest `role` puts them in an AI slot (`APP_MANIFEST.md` # D4 # Roles and requires) leave the Settings row and show as provider tiles, in the order of the catalog's provider data, the first five first and the rest behind **More**, with "Other (OpenAI-compatible)" last. A tile shows for a slot when its provider can fill it: the app's native slot for the provider's protocol first, else the app's OpenAI-compatible slot when the provider has an OpenAI-compatible address (Other always does). A tile is also hidden when the provider has no model of a type the slot asks for. Picking a tile lists the user's own AI accounts for that provider; a sole one is preselected, and **Add an account** opens a short form right there (name, key with the provider's key link and a soft warning when the key does not start with the usual prefix, and the server address for Other). The new account is saved for the user without a password re-prompt, and picked. Then the page shows a model picker for each model setting the slot declares: one choice for `model.<type>`, several for `models.<type>`, with the provider's models of that type, its suggested one first and chosen, and a box for a model name that is not in the list. An app can take several providers at once, one per slot. The compatible slot holds one provider, so a second pick replaces the first, and the page says so. The page sends each filled slot as a binding (`config.ai_bindings`, `BRAIN_UI_PROTOCOL.md` # POST /api/v1/apps), never the key, and the brain fills the fields. A native slot that no provider can fill keeps its fields in the Settings row. With no provider data there is no AI row, and every field is a plain field. **The app's `requires` groups gate the Install button** like required fields: a filled slot counts, and the line next to the button says what is still needed ("an AI provider").
