...
**Mail-capable apps add an outgoing-email picker to the consent dialog.** When the manifest declares `mail:` (`APP_MANIFEST.md` # D3), the install plan carries the box's registered providers (id + label) and the dialog offers a radio pick: **None** (default — the app installs with email features off) or one of the providers; a sole registered provider is preselected as the obvious intent. The binding is changeable later from the app's detail page, where a rebind re-stamps the env and recreates the containers (`SERVICE_PROVISIONING.md` # BYO outgoing mail).

**Apps with `config:` add a setup-fields step to the consent dialog.** When the manifest declares user-supplied configuration (`APP_MANIFEST.md` # D4), the install plan carries each field's schema (`app_env`, `title`, `description`, `secret`, `required`, `type`, `options`, `default`) and the dialog renders a small form: each field shows its `title` and `description` prominently with the `app_env` as a monospace hint beneath ("Sets `OPENAI_API_KEY`") so a user reading the app's own docs can confirm the match. Secret fields render as masked inputs; `enum` as a select, `bool` as a toggle, `text` as a single-line input. **Required fields gate the Install button** — it stays disabled until every required field has a value, so the app never installs into a guaranteed crash-loop; optional fields left blank are simply not injected (the app keeps its own default). The answers ride the same `POST /api/v1/apps` `config` body as the folder/scope elections and are stamped into the compose override (`BRAIN_UI_PROTOCOL.md` # POST /api/v1/apps). Values are editable later from the app's detail page (# Installed apps).
