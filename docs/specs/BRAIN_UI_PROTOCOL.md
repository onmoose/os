...
An install binds an app's AI slots to these accounts with `config.ai_bindings` on `POST /api/v1/apps` (below). Deleting an account removes its bindings. The app keeps the values it was given until its settings are next written.
...
```jsonc
{
  ...
  "config": [                          // user-supplied config fields (APP_MANIFEST.md # D4); omitted when the manifest declares none
    ...
    },
    {
      ...
      "description": "...",
      "type": "text",
      "role": "ai.openai_compatible.models.chat",  // only when the box can fill the field
      "separator": ";"                            // only on a fillable models.<type> field
    }
  ],
  "requires": [                        // "at least one of" groups the box checks; omitted when none
    { "one_of": ["ai"] }
  ]
}
```
...
- **Roles and requires.** A config field carries `role` (`APP_MANIFEST.md` # D4 # Roles and requires) only when the box can fill it: a known kind, attribute and model type, a valid separator, `type: text`, a key field that is `secret`, and not a repeat of an earlier field's role. Any other field has no `role` and is a plain field. `separator` is the effective separator (the declared one, or `,`) and is sent only on a fillable `models.<type>` field. `requires` lists the groups the box checks, each `{ "one_of": [...] }` with members that are a kind (`ai`), a slot (`ai.anthropic`) or a plain field's `app_env`. A member that matches no field is left out, and so is a group left empty, so every group the UI sees can be met. This endpoint does not say which provider can fill a slot: the setup page works that out from the roles and `GET /api/v1/ai-providers` (`INSTALL_SETUP.md` # 1).
- **Mail picker is the caller's own accounts.** For an app with a manifest `mail:` block, `mail` carries `optional` and `providers`: the caller's own email accounts as `{id, label, provider_type}`, never another user's and never a credential. They are the only accounts `POST /api/v1/apps` accepts in `config.mail_provider_id`; another user's id gets the same `422 no such mail provider` as a missing one (`SERVICE_PROVISIONING.md` # BYO outgoing mail). `GET /api/v1/mail-providers/options` serves the same list for the rebind picker on an installed app's settings screen, and `PUT /api/v1/apps/{id}/mail-binding` applies the same rule.
...
**The install `config` body carries the user's elections — folder sources, subfolders, mail binding, and user-supplied config values.** Setup-field answers (`APP_MANIFEST.md` # D4) arrive keyed by `app_env`, e.g. `"config": { ..., "fields": { "OPENAI_API_KEY": "sk-…", "OPENAI_MODEL": "gpt-4o-mini" } }`. The brain validates against the manifest (every `required` field present and non-empty, `enum` values within `options`, no unknown `app_env`, and every `requires` group met) → `422` with per-field `errors` on failure; on success it stamps each value into the target service's `environment:` in the compose override, under the `app_env` verbatim, before the first `compose up`. Secret-field values are persisted like `MOOSE_SECRET_*` and never returned by any read endpoint.

**`config.ai_bindings` fills AI slots from the caller's AI accounts** (`INSTALL_SETUP.md` # 1 and # 5). It is a list of `{ "slot": "ai.anthropic", "account_id": "…", "models": { "model.chat": ["claude-sonnet-5"], "models.embedding": ["a", "b"] } }`, one per slot. `slot` is `kind.protocol` from the manifest's roles, and `models` maps a model setting the slot declares to model ids: exactly one for `model.<type>`, one or more for `models.<type>`. A setting left out gets the provider's default for its type. A model id that is not in the provider data is accepted. The brain resolves each binding into the slot's `app_env` values: `api_key` gets the account's key (nothing for a keyless OpenAI-compatible account); a compatible `base_url` gets the account's base URL, else the provider's `openai_base_url`; a native `base_url` gets the account's base URL or nothing; each model setting gets its ids, a list joined with the field's separator. The resolved values are stored and injected like typed values, with the field's secret flag, and the bindings are stored too (`instance_ai_bindings`). Every refusal is a `422` that audits `app.install` with `success=false`:

- `config.ai_bindings: this app has no AI slot "<slot>"`, and `config.ai_bindings: slot <slot> is given more than once`.
- `config.ai_bindings: pick an AI account for slot <slot>`, and `config.ai_bindings: no such AI account` for a missing id or another user's (a household app uses the installing admin's accounts).
- `config.ai_bindings: the account "<label>" does not work with this app's <slot> slot`: a native slot needs a provider with that `native_protocol`, and the compatible slot an `openai_compatible` account or a provider with an `openai_base_url`.
- For an account whose provider has left the provider data: `config.ai_bindings: the provider of the account "<label>" is not in the provider list now, so it cannot fill the <slot> slot` on a native slot, and `… is not in the provider list now. Give the account its own base URL, or pick another account` on the compatible slot when the account has no base URL of its own.
- `config.ai_bindings: the <slot> slot of this app does not take <setting>` for a `models` key the slot does not declare.
- `config.ai_bindings: pick a model for <title>` when a setting has no ids and the provider has no default for its type (an `openai_compatible` account never has one); `pick exactly one model for <title>`; `pick at least one model for <title>`; `a model name for <title> is empty`; `a model name for <title> must not contain line breaks or control characters`; `model "<id>" is picked twice for <title>`; `model "<id>" for <title> contains "<sep>", which this app uses to separate models`.
- `config.fields: <APP_ENV> is filled from an AI account, so do not also send a value for it`: a bound slot's fields may not also get a non-empty typed value. Typed values for role fields without a binding stay allowed, so an install works with no provider data.

**`requires` is checked on the resolved values.** The values a binding filled count. A group is met when at least one of its fields has a non-empty value after the answers are resolved; model fields never count (`APP_MANIFEST.md` # D4 # Roles and requires). The first unmet group is a `422`. A group that is exactly the kind `ai` answers `config.fields: pick at least one AI provider`; any other group lists the titles of its fields, `config.fields: fill in at least one of: Search key, Search server`. Like every other rejected install election, it audits `app.install` with `success=false`.

**`GET`/`PUT /api/v1/apps/{id}/config` edit the values after install.** `GET` returns the field schema plus current non-secret values; a secret field reports `"set": true|false` but never its value. **`PUT` is a partial update, not a full replace** — this matters because the client can never read a secret back, so it cannot resubmit it. An `app_env` **omitted** from the request keeps its stored value; a field present with a non-empty value sets it; a field present with an explicit empty string (an optional field only) clears it. **`required` is validated against the resulting state, not the request** — a required field is satisfied by a value already stored, so editing one field never forces the user to re-enter a secret they can't see, and a required secret can only be *replaced* (by sending a new value), never accidentally blanked (an empty string on a required field is a `422`). The same per-field validation as install otherwise applies (enum within `options`, no unknown or reserved `app_env`). **`requires` is checked as "no worse":** an edit that leaves a group unmet which was met before it is a `422`, `config.fields: keep at least one AI provider` for the `ai` kind group or `config.fields: keep at least one of these filled in: <titles>` for any other. A group that was already unmet (an app installed before `requires`, or before this check) does not block an unrelated edit. `GET` returns each field's `role` and `separator` and the top-level `requires` groups, with the same meaning as on the install plan. `PUT` rewrites the override and restarts the app so the new environment takes effect — a job, since `compose up` recreates containers. Both follow the app-control gate (admin for any app, owner for a personal app — the same gate as stop/start and the secret reveal); a config change is an elevation-class mutation and audits success and failure (`CONTROL_PLANE.md`).
