...
| 2026-09-26 | Piece 3: `POST /api/v1/apps` carries `config.ai_bindings`, a list of `{slot, account_id, models}`. `slot` is `kind.protocol` from the manifest. `models` maps a model attribute the slot declares (`model.chat`, `models.embedding`) to a list of model ids: exactly one for `model.<type>`, one or more for `models.<type>`. A typed model id not in the provider data is accepted. |
| 2026-09-26 | The brain resolves a binding into the slot's `app_env` values. The account must be the caller's, as for email (a household app uses the installing admin's accounts). A native slot takes an account whose provider has that `native_protocol`. The compatible slot takes an `openai_compatible` account, or one whose provider has an `openai_base_url`. `api_key` gets the account's key. A compatible `base_url` gets the account's base URL, or else the provider's `openai_base_url`. A native `base_url` gets the account's base URL or stays blank. Each declared model attribute gets the chosen ids, or else the provider's default for that type. With neither, the install is a 422. A `models` list is joined with the field's separator, and an id that contains the separator is a 422. |
| 2026-09-26 | A slot is filled either by a binding or by plain values, never both in one install (422). Plain values for role fields stay possible, so an install still works with no provider data. `requires` is checked on the resolved values. |
| 2026-09-26 | The brain stores each binding (instance, slot, account, models) so piece 4 can show the pickers and rebind on a key change. The resolved values are also stored as the app's config values, so the compose override works as today. Deleting an account removes its bindings; what the app shows then is piece 4. |
| 2026-09-26 | The setup page reads roles instead of env names, and the env-name lookup in `aiProviders.ts` goes. A tile shows for a slot when the rule in # 1 lets its provider fill it and, for a listed provider, when it has a model of every type the slot declares. The user picks one of their accounts for that provider or adds one inline, then the models. Install stays disabled until every `requires` group is met. |
...
**As built (roles in the schema).** `role` and `separator` are on `ConfigField`, and the vocabulary is in `internal/manifest/roles.go` (`ParseRole`, `FillableRoles`, `EffectiveSeparator`). The box reads them leniently and the `moose manifest` lint strictly (`internal/manifest/lint.go`); `APP_MANIFEST.md` # D4 # Roles and requires has the schema. The install plan and the config endpoint send each fillable field's `role` and `separator`.

**As built (filling a slot, piece 3).** The setup page groups role fields into slots and draws tiles with the rule above (`web-ui/src/aiProviders.ts`), and hides a listed provider's tile for a slot when it has no model of a type the slot declares. `POST /api/v1/apps` takes `config.ai_bindings`, and the brain resolves each binding into the slot's values (`internal/api/aibindings.go`). The values are checked like typed values, so `required` and `requires` see them, and stored as the app's config values. `BRAIN_UI_PROTOCOL.md` # POST /api/v1/apps lists the rules and every 422.
...
**As built (AI accounts, piece 2).** The `ai_accounts` table (`internal/store/aiaccounts.go`) and `GET`/`POST /api/v1/ai-accounts` and `PUT`/`DELETE /api/v1/ai-accounts/{id}` (`internal/api/aiaccounts.go`) follow the 2026-09-26 rows above. `SERVICE_PROVISIONING.md` # AI provider accounts has the rules and `BRAIN_UI_PROTOCOL.md` # AI provider accounts the wire shape.

**As built (bindings, piece 3).** An install binds each filled slot to one of the installer's accounts. The brain stores the binding in `instance_ai_bindings` (instance, slot, account, the model ids after defaults; `internal/store/aiaccounts.go`), next to the resolved config values. A binding goes with its app on uninstall and with its account on delete, and a failed user delete puts it back. Rebinding when a key changes, the pickers on the app's settings screen, and what the app shows after its account is deleted are piece 4. Until then a deleted account's values stay in the app until its settings are next written.
...
**As built (AI row, piece 3).** The AI row reads roles, not env names: the env-name lookup is gone. Picking a tile lists the user's accounts for that provider (Other lists `openai_compatible` accounts) and offers an inline add (name, key, and the server address for Other) that saves the account without a password re-prompt and picks it. Then one model picker per model setting: single for `model.<type>`, several for `models.<type>`, the provider's default first and chosen, and a typed id. The page sends bindings, not the slot's fields. Install stays disabled until every `requires` group is met, counting a bound slot as filled, and the line next to the button names what is missing. `DASHBOARD.md` # Install authorization describes the row.
