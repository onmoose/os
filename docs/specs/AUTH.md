...
- Passkeys are origin-bound by design. A passkey on `moose.local` doesn't work on `cindy-zx9.onmoose.network`. With the toggle that flips schemes, users would re-enroll per origin — terrible UX.
...
| `HttpOnly`  | yes                                                                   |
| `Secure`    | yes on `.onmoose.network`, no on `moose.local` (HTTP-only there)        |
| `SameSite`  | `Lax`                                                                 |
...
- On `cindy-zx9.onmoose.network`, they have a separate `moose_session` cookie for `cindy-zx9.onmoose.network`. Different cookie, different session row server-side.
