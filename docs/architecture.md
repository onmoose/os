...
| **Caddy** | `dev/caddy.json`, `dev/docker-compose.yml` | Reverse proxy. Terminates `*.local` (appliance) or `*.<box-id>.onmoose.network` over real Let's Encrypt HTTPS (hosted, via a custom acme-dns build) and routes to app containers + the brain. Configured live by the brain via Caddy's admin API. | Real (container) |
...
- **Boot, install ISO, updates.** The `mkosi` image build (`BUILD.md` # 2;
  ...
  **appliance** the source is the signed release manifest and the box only
  ...
  signing key and no `releases.onmoose.network`** (so every build refuses every
